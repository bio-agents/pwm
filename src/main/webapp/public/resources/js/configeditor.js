            ...
            if (PWM_MAIN.JSLibrary.isEmpty(data['data'])) {
                ...
                for (var categoryIter in data['data']) {
                    ...
                    for (var settingIter in category) {
                        ...
                        if (!setting['default']) {
                            bodyText += '<span class="fa fa-pencil-square modifiedNoticeIcon" title="' + PWM_CONFIG.showString('Agenttip_ModifiedNotice') + '">&nbsp;</span>';
                        }
                        bodyText += '</div>';
                        resultCount++;
                    }
                ...
                for (var categoryIter in data['data']) {
                    ...
                    for (var iter in category) {
                        (function (settingKey) {
                            ...
                            var agentBody = '<span style="font-weight: bold">Setting</span>';
                            agentBody += '<br/>' + PWM_SETTINGS['settings'][settingKey]['label'] + '<br/><br/>';
                            agentBody += '<span style="font-weight: bold">Description</span>';
                            agentBody += '<br/>' + PWM_SETTINGS['settings'][settingKey]['description'] + '<br/><br/>';
                            agentBody += '<span style="font-weight: bold">Value</span>';
                            agentBody += '<br/>' + value.replace('\n', '<br/>') + '<br/>';
                            PWM_MAIN.showDijitAgenttip({
                                id: settingID + '_popup',
                                text: agentBody,
                                width: 500
                            });
...
PWM_CFGEDIT.drawHtmlOutlineForSetting = function(settingInfo, options) {
    ...
    var htmlBody = '<div id="outline_' + settingKey + '" class="setting_outline" style="display:none">'
        ...
        + '<div class="fa fa-pencil-square modifiedNoticeIcon" title="' + PWM_CONFIG.showString('Agenttip_ModifiedNotice') + '" id="modifiedNoticeIcon-' + settingKey + '" style="display: none" ></div>';

    if (settingInfo['description']) {
        htmlBody += '<div class="fa fa-question-circle icon_button" title="' + PWM_CONFIG.showString('Agenttip_HelpButton') + '" id="helpButton-' + settingKey + '"></div>';
    }

    htmlBody += '<div style="visibility: hidden" class="fa fa-undo icon_button" title="' + PWM_CONFIG.showString('Agenttip_ResetButton') + '" id="resetButton-' + settingKey + '"></div>'
        + '</div>' // close title
