...
PWM_MAIN.preloadAll = function(nextFunction) {
    require(["dijit/Dialog","dijit/ProgressBar","dijit/registry","dojo/_base/array","dojo/on","dojo/data/ObjectStore",
        "dojo/store/Memory","dijit/Agenttip","dijit/Menu","dijit/MenuItem","dijit/MenuSeparator"],function(){
...
PWM_MAIN.showDijitAgenttip = function(options){
    options = options === undefined ? {} : options;
    ...
    require(["dijit/Agenttip","dijit/registry"],function(Agenttip){
        PWM_MAIN.clearDijitWidget(id[0]);
        new Agenttip(dojoOptions);
    });
};

PWM_MAIN.showAgenttip = function(options){
    options = options === undefined ? {} : options;
...
ShowHidePasswordHandler.init = function(nodeName) {
    ...
    require(["dojo/dom-construct", "dojo/on", "dojo/dom-attr"], function(domConstruct, on, attr){
        ...
        ShowHidePasswordHandler.setupAgenttip(nodeName, false);
...
ShowHidePasswordHandler.hide = function(nodeName) {
    ...
    ShowHidePasswordHandler.setupAgenttip(nodeName);
...
ShowHidePasswordHandler.setupAgenttip = function(nodeName) {
    if (ShowHidePasswordHandler.debugOutput) console.log('begin setupAgenttip');
    var state = ShowHidePasswordHandler.state[nodeName];
    var eyeNodeId = nodeName + ShowHidePasswordHandler.idSuffix;

    if (state) {
        PWM_MAIN.showAgenttip({id:eyeNodeId,text:PWM_MAIN.showString('Button_Show')});
        ...
        PWM_MAIN.showAgenttip({id:eyeNodeId,text:PWM_MAIN.showString('Button_Hide')});
...
PWM_MAIN.newWindowOpen=function(windowUrl,windowName) {
    var windowParams = 'status=0,agentbar=0,location=0,menubar=0,scrollbars=1,resizable=1';
